async function postA2EActivationAccount(data = {}) {
  // Default options are marked with *
  try {
    const url =
      'https://test.a2eship.com/api/a2e-shopify/v1/customer/activation';
...
async function callA2EActivationAccount(shopifyCustomerId, password) {
  const data = { shopifyCustomerId, password };
  try {
    const is_verify = await postA2EActivationAccount(data);
    return is_verify;
  } catch (err) {
    alert(err.message);
  }
...
async function onSubmitActivation(form) {
  alert('hello a2e activation ');
  ...
  if (customerId) {
    startPageLoader();
    const isVerify = await callA2EActivationAccount(customerId, pass);
    stopPageLoader();
